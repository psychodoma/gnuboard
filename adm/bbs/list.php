...
if ($board['bo_use_category']) {
    ...
    if ($sca==''){
        $category_option .= '<li class="active"><a href="'.$category_href.'"';
    ...
    for ($i=0; $i<count($categories); $i++) {
        ...
        if ($category==$sca) { // 현재 선택된 카테고리라면
            $category_option .= '<li class="active"><a href="'.($category_href."&amp;sca=".urlencode($category)).'"';
            $category_option .= ' id="bo_cate_on" ';
...
$write_pages = kim_get_paging(G5_IS_MOBILE ? $config['cf_mobile_pages'] : $config['cf_write_pages'], $page, $total_page, './board.php?bo_table='.$bo_table."&".$qstr.'&amp;page=','&sca='.$sca);
...
include_once(kim_get_skin_path('board', $board['bo_skin']).'/list.admin.php');
?>
