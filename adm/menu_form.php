...
<div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
        <div class="x_content">
            <p class='bg-info' style='padding:15px;' ><?php echo $g5['title']; ?></p>

                <form name="fmenuform" id="fmenuform">

                <div class="new_win_desc">
                    <label for="me_type">대상선택</label>
                    <select class='form-control' name="me_type" id="me_type">
...
function add_menu_list(name, link, code)
{
    ...
    list += "<label for=\"me_name_"+ms+"\"  class=\"sound_only\">메뉴<span class=\"required red\"> *</span></label>";
    list += "<input type=\"hidden\" name=\"code[]\" value=\"<?php echo $code; ?>\">";
    list += "<input type=\"text\" name=\"me_name[]\" value=\""+name+"\" id=\"me_name_"+ms+"\" required class=\"required frm_input form-control full_input\">";
    list += "</td>";
    list += "<td>";
    list += "<label for=\"me_link_"+ms+"\"  class=\"sound_only\">링크<span class=\"required red\"> *</span></label>";
    list += "<input type=\"text\" name=\"me_link[]\" value=\""+link+"\" id=\"me_link_"+ms+"\" required class=\"required form-control frm_input full_input\">";
    list += "</td>";
    list += "<td class=\"td_mng\">";
    list += "<label for=\"me_target_"+ms+"\"  class=\"sound_only\">새창</label>";
    list += "<select class=\"form-control\" name=\"me_target[]\" id=\"me_target_"+ms+"\">";
    list += "<option value=\"self\">사용안함</option>";
    list += "<option value=\"blank\">사용함</option>";
    list += "</select>";
    list += "</td>";
    list += "<td class=\"td_numsmall\">";
    list += "<label for=\"me_order_"+ms+"\"  class=\"sound_only\">순서<span class=\"required red\"> *</span></label>";
    list += "<input type=\"text\" name=\"me_order[]\" value=\"0\" id=\"me_order_"+ms+"\" required class=\"required form-control frm_input\" size=\"5\">";
    list += "</td>";
    list += "<td class=\"td_mngsmall\">";
    list += "<label for=\"me_use_"+ms+"\"  class=\"sound_only\">PC사용</label>";
    list += "<select class=\"form-control\" name=\"me_use[]\" id=\"me_use_"+ms+"\">";
    ...
    list += "<select class=\"form-control\" name=\"me_mobile_use[]\" id=\"me_mobile_use_"+ms+"\">";
    list += "<option value=\"1\">사용함</option>";
    list += "<option value=\"0\">사용안함</option>";
    list += "</select>";
    list += "</td>";
    list += "<td class=\"td_mngsmall\" style=\"vertical-align: inherit;\">";
    <?php if($new == 'new') { ?>
    list += "<button type=\"button\" class=\"btn_add_submenu btn btn-default\">추가</button>";
    <?php } ?>
    list += "<button type=\"button\" class=\"btn_del_menu btn btn-default\">삭제</button>";
    list += "</td>";
    list += "</tr>";
