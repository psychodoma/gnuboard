...
$g5['title'] = '접속기기별(PC,Mobile,Tablet) 접속자집계';
...
while ($row=sql_fetch_array($result)) {
	...
	if($detect->isMobile() && ! $detect->isTablet()) {
        ...
        foreach($detect->getPhoneDevices() as $key => $value) { 
			$check = $detect->{'is'.$key}();
            if($check) {
	           $arr[$key]++;
        ...
        foreach($detect->getTabletDevices() as $key => $value) { 
			$check = $detect->{'is'.$key}();
            if($check) {
	           $arr[$key]++;
               if ($arr[$key] > $max) $max = $arr[$key];
            ...
            <div class="clearfix"></div>
                ...
                <div class="x_content">
                    ...
                    <table id="datatable-buttons" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                <th class="col-md-1" scope='col'>순위</th>
                                <th class="col-md-2" scope='col'>브라우저</th>
                                <th class="col-md-5" scope='col'>그래프</th>
